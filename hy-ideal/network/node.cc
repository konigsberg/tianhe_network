...
double delays[10] = {0, 100, 0, 300, 0, 500, 0, 800, 0, 1000};
...
void Node::gen_timer_cb() {
  if (debug_mode && (total_flit_sent_ >= 400 || getIndex() >= 10))
    return;
...
bool Node::channel_is_available() {
  ...
  return finish_time <= omnetpp::simTime();
}
